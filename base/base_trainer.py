...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, resume, config, train_logger=None):
        ...
        model_dir = '-'.join([config['data_loader']['args']['data_dir'].split('/')[-1],
                              'ss_%s' % ss,
        ...
        self.checkpoint_dir = os.path.join(cfg_trainer['save_dir'], config['name'], model_dir)
        # setup visualization writer instance
        writer_dir = os.path.join(cfg_trainer['log_dir'], config['name'], model_dir)
    ...
    def _save_checkpoint(self, epoch):
        ...
        """
        state = self._extract_info(epoch)
